...
namespace ExtraDetailingTools
{
	internal partial class TransformSection : InfoSectionBase
	{
		...
		protected override void OnUpdate()
		{
			...
			if (visible)
			{
				...
				//if (!EntityManager.HasBuffer<MeshColor>(selectedEntity)) EntityManager.AddBuffer<MeshColor>(selectedEntity);
				//DynamicBuffer <MeshColor> meshColors = EntityManager.GetBuffer<MeshColor>(selectedEntity);
				//meshColors.Clear();
				//MeshColor meshColor = new()
				//{
				//	m_ColorSet = new(UnityEngine.Color.red),
				//};
                ...
                if (!EntityManager.HasComponent<TransformObject>(selectedEntity))
				{
					...
					TransformObject transformObject = new(new(2f, 2f, 2f));
					EntityManager.SetComponentData(selectedEntity, transformObject);


					//if (EntityManager.TryGetComponent(selectedEntity, out PrefabRef prefabRef) && EntityManager.TryGetComponent(prefabRef.m_Prefab, out ObjectGeometryData objectGeometryData) && EntityManager.TryGetComponent(selectedEntity, out CullingInfo cullingInfo))
					//{
						//objectGeometryData.m_Size *= transformObject.m_Scale;
						//objectGeometryData.m_Bounds *= transformObject.m_Scale;
						//EntityManager.SetComponentData(prefabRef.m_Prefab, objectGeometryData);

                        //Bounds3 bounds3 = ObjectUtils.CalculateBounds(transform.m_Position, transform.m_Rotation, objectGeometryData.m_Bounds * transformObject.m_Scale);
                        //cullingInfo.m_Bounds = bounds3;
                        //EntityManager.SetComponentData(selectedEntity, cullingInfo);

						//if(EntityManager.TryGetBuffer(prefabRef.m_Prefab, false, out DynamicBuffer<SubMesh> subMeshs))
						//{
						//	for(int i = 0; i < subMeshs.Length; i++)
						//	{
						//		SubMesh subMesh = subMeshs[i];
						//		DynamicBuffer<MeshVertex> meshVertices = EntityManager.GetBuffer<MeshVertex>(subMesh.m_SubMesh, false);
						//		for (int j = 0; j < meshVertices.Length; j++)
						//		{
						//			MeshVertex meshVertex = meshVertices[j];
						//			meshVertex.m_Vertex *= transformObject.m_Scale;
						//			meshVertices[j] = meshVertex;
      //                          }
						//	}
						//}
                    //}
				...
				if (showAxis)
				{
					RenderAxisJob renderAxisJob = new()
					{
						...
						m_TransformObjectLookup = SystemAPI.GetComponentLookup<TransformObject>(),
	...
	[BurstCompile]
		...
		private struct RenderAxisJob : IJob
		{
			...
			public ComponentLookup<TransformObject> m_TransformObjectLookup;
			public ComponentLookup<PrefabRef> m_PrefabRefLookup;
			public ComponentLookup<ObjectGeometryData> m_ObjectGeometryDataLookup;
			public float3 rot;
			...
			public void Execute()
			{
				...
				{
					bounds3 = useLocalAxis ? geometryData.m_Bounds : ObjectUtils.CalculateBounds(m_Transform.m_Position, m_Transform.m_Rotation, geometryData);
				}

				float3 linesLenght = new(bounds3.x.max - bounds3.x.min, bounds3.y.max - bounds3.y.min, bounds3.z.max - bounds3.z.min);

				if(m_TransformObjectLookup.TryGetComponent(m_SelectedEntity, out TransformObject transformObject))
				{
					linesLenght *= transformObject.m_Scale;
				}

				float3 xAxis = new(linesLenght.x, 0f, 0f);
				float3 yAxis = new(0f, linesLenght.y, 0f);
				float3 zAxis = new(0f, 0f, linesLenght.z);
				float3 pos = m_Transform.m_Position;
				if (useLocalAxis)
				{
