...
internal partial class EDT_BatchDataSystem : GameSystemBase
{
    ...
    private struct ScalMeshJob : IJobParallelForDefer
    {
        ...
        {
            PreCullingData preCullingData = this.CullingDatas[index];
            if (!MeshBatcheLookup.TryGetBuffer(preCullingData.m_Entity, out DynamicBuffer<MeshBatch> MeshBatchBuffer))
            {
                return;
            }
